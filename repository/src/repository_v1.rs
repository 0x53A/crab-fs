...
fn visit_symlink_internal(&self, inode: InodeEntry, symlink: OsStr, visit: Box<dyn RepositoryVisitor>) {
    visit.on_inode(inode);
}

fn visit_file_internal(&self, inode: InodeEntry, file: FileContent, visit: Box<dyn RepositoryVisitor>) {
    visit.on_inode(inode);
}

fn visit_dir_internal(&self, inode: InodeEntry, dir: DirectoryDescriptor, visit: Box<dyn RepositoryVisitor>) {
    ...
    for e in dir {
        ...
        match (child_kind, child_inode_entry) {
            ...
            _ => {
                visit.on_mismatching_inode_type()
            }
        }
    }
}
...
fn on_inode(path: Vec<Vec<u8>>, inode_entry: InodeEntry);
fn on_missing_inode_file(path: Vec<Vec<u8>>, inode: Inode);
//fn on_deserialization_error(inode: Inode);
fn on_mismatching_inode_type(inode_entry: InodeEntry);
}
