...
impl FS for PhysicalFS {
    ...
    fn copy_file_range(
        ...
        return Ok(copy_file_range(src_file, src_offset, dst_file, dst_offset, len)?);
    }
}
...
impl Read for DummyFile {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        Err(io::Error::new(io::ErrorKind::Other, "This is a dummy file and can't be used"))
    }
}
...
impl<F:RemoteFs> Read for RemoteFsFile<F> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        match &mut self.read_stream {
            ...
            None => Err(io::Error::new(io::ErrorKind::NotFound, "File not opened for reading"))
        }
    }
}
...
impl<F:RemoteFs> Write for RemoteFsFile<F> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        match &mut self.write_stream {
            ...
            None => Err(io::Error::new(io::ErrorKind::NotFound, "File not opened for writing"))
        }
    }
...
impl<F:RemoteFs> Seek for RemoteFsFile<F> {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        if let Some(stream) = self.write_stream.as_mut() {
            ...
            return Err(io::Error::new(io::ErrorKind::NotFound, "File not opened for reading or writing"));
        }
    }
}
...
impl<F:RemoteFs + Capabilities> SetLen for RemoteFsFile<F> {
    ...
    fn set_len(&mut self, size: u64) -> MyResult<()> {
        ...
        if current_len > size {
            ...
            if !fs.can_append() {
                return Err(io::Error::new(io::ErrorKind::Other, "Cannot grow file - append not supported").into());
            }
...
impl<T: RemoteFs + Copy + Capabilities> FS for RemoteFsAdapter<T> {
    ...
    fn create_dir_all<P: AsRef<Path>>(&self, path: P) -> MyResult<()> {
        ...
        match self.inner.borrow_mut().create_dir(path, UnixPex::from(0o755)) {
            Ok(_) => Ok(()),
